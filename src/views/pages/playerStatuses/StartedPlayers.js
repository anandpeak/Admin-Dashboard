...
import { GET_STARTED_PLAYER_INFO } from 'src/apollo/useQuery'
import { useQuery } from '@apollo/client'
import { HashLoader } from 'react-spinners'

const CompletedPlayers = () => {
  const { data, loading, error } = useQuery(GET_STARTED_PLAYER_INFO)
  ...
  useEffect(() => {
    if (data) {
      const allPlayerInfo = data.GetStartedPlayerInfo
  ...
  return (
    <div>
      <CCard>
        <CCardHeader>Нийт дуусгалгүй гарсан хүмүүсийн хүснэгт</CCardHeader>
        <CCardBody>
          <div className="mb-4">
            <input
              type="text"
